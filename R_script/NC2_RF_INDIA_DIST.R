NC_RF_INDIA_DIST <- function(out.dir,ncfile, DAYY,TIME,NDAYS,nv_time,stat,end,stat_rf,end_rf){
...
pdf(paste("NC2PDF/RAINFALL_INDIA_DIST_",format(as.Date(DAYY), "%Y_%b_%d "),TIME,".pdf",sep = ""), width = 10 , height =7.5 ,onefile=T,bg="white")
...
grid.text(paste("*Rainfall forecast: Accumulated rainfall from 05:30 AM to 05:30 AM (next day) IST",sep = ""), x=.05, y=.04, gp=gpar(fontsize=8) , check=TRUE,just = "left")
...
png(paste("NC2PDF/RAINFALL_INDIA_DIST_",format(as.Date(DAYY), "%Y_%b_%d "),TIME,".png",sep = ""), width = 10000 , height =7500)
# RTG <- readPNG("../src_files/RTGC_English_Telugu_12x1.png")
# grid.raster(RTG[,1:700,],x=0.1,y=.96, width=unit(.17, "npc"), interp=FALSE)
# grid.raster(RTG[,1700:2400,],x=0.9,y=.96, width=unit(.17, "npc"), interp=FALSE)
...
grid.text("Rainfall forecast ", x=.5, .96, gp=gpar(fontsize=220,fontface = "bold") , check=TRUE)
...
grid.text(paste(" Source: Experimental predictions from ISRO-SDSC SHAR based on European Centre for Medium Range Weather forecast System (ECMWF)",sep = ""), x=.05, y=.02, gp=gpar(fontsize=120) , check=TRUE,just = "left")
grid.text(paste("*Rainfall forecast based on ",format(as.Date(DAYY), "%Y_%b_%d "),TIME,sep = ""),x=.05, y=.90, gp=gpar(fontsize=140,fontface = "bold") , check=TRUE,just = "left")
