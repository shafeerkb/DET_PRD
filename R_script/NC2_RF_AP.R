NC_RF_AP <- function(out.dir,ncfile, DAYY,TIME,NDAYS,nv_time,stat,end,stat_rf,end_rf){
...
print(dd)
  ...
  image(RF[,144:1],x=lon,y=rev(lat),breaks = lvlsmn,main="", col = collmn,axes=F,xlab = "",ylab = "")
  box()
...
pdf(paste("NC2PDF/RAINFALL_AP_",format(as.Date(DAYY), "%Y_%b_%d "),TIME,".pdf",sep = ""), width = 10 , height =7.5 ,onefile=T,bg="white")
