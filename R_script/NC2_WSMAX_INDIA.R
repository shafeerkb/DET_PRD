NC_WSMAX_INDIA <- function(out.dir,ncfile, DAYY,TIME,NDAYS,nv_time,stat_swh,end_swh,stat_rf,end_rf){
...
pr <- ncvar_get( f1,"wind_magnitude") *3.6
...
collmn=colorRampPalette(c("#FFFFFF", "#4795B1","#F5E75B","#D82928","#921519"))( 25) 
lvlsmn=seq(0,125,5)
...
pdf(paste("NC2PDF/MAX_WS_INDIA_",format(as.Date(DAYY), "%Y_%b_%d "),TIME,".pdf",sep = ""), width = 10 , height =7.5 ,onefile=T,bg="white")
...
RF <- readPNG("TEMP.png")
VASAR <- readPNG("../src_files/seastate_scale_BOB_ws.png")


grid.raster(RF,x=0.47,y=.47, width=unit(0.9, "npc"), interp=FALSE)
grid.raster(VASAR,x=0.73,y=.19, width=unit(0.45, "npc"), interp=FALSE)


grid.text("Wind Speed (km/hr) forecast ", x=.5, .96, gp=gpar(fontsize=18,fontface = "bold") , check=TRUE)
...
grid.text(paste("*Maximum Wind Speed from (02:30,05:30,08:30,11:30,14:30,17:30,20:30,23:30 IST) time steps",sep = ""), x=.05, y=.04, gp=gpar(fontsize=8) , check=TRUE,just = "left")
...
grid.text("(km/hr)", x=.97, .85, gp=gpar(fontsize=12,fontface = "bold") , check=TRUE)
for (i in 1:length(collmn)){
  grid.rect(width = .025, height = 0.035,x=.945,y=.047+(i*.03),  gp=gpar(col=NA, fill=collmn[i]))
  if(i!=length(collmn)){grid.text(lvlsmn[(i+1)], x=.97,y= .057+(i*.03), gp=gpar(fontsize=11,fontface = "bold",just = "left") , check=TRUE)}
}
